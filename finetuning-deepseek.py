...
class LoRAModel(nn.Module):
    ...
    def merge_lora_weights(self):
        """Permanently merge LoRA weights. Only use for final save."""
...
def setup_logging(accelerator, project_name, args):
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        handlers=[
            logging.FileHandler(log_dir / f"debug_deepseek_{accelerator.process_index}.log"),
    ...
    if accelerator.is_main_process:
        if args.use_wandb:
            ...
            run_name = 'local_run'
        tb_writer = SummaryWriter(log_dir / "tensorboard_deepseek")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Finetune DeepSeek-Coder-6.7B with LoRA")

    # Model parameters
    parser.add_argument("--model_name", type=str, default=os.path.expanduser("~/models/deepseek-coder-6.7b-instruct"), help="Base model path")
    ...
    parser.add_argument("--output_dir", type=str, default="./results_deepseek", help="Output directory")
    parser.add_argument("--project_name", type=str, default="COREGEN-DeepSeek", help="Project name")
...
def main():
    ...
    # Load DeepSeek-Coder-6.7B model (standard LLaMA architecture, 32 layers)
    ...
    # DeepSeek-Coder V1 6.7B uses standard LLaMA architecture with q/k/v/o_proj
