...
import datasets
import transformers
from COREGEN7b import LlamaForCausalLM as COREGENLlamaForCausalLM
...
def setup_logging(accelerator, project_name, args):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_dir / f"debug_deepseek_s{args.stage}_{accelerator.process_index}.log"),
    ...
    if accelerator.is_main_process:
        ...
        tb_writer = SummaryWriter(log_dir / f"tensorboard_deepseek_s{args.stage}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="COREGEN two-stage training for DeepSeek-Coder-6.7B")
    ...
    parser.add_argument("--model_name", type=str, default=None,
                        help="Model path. Stage 1 default: ~/models/deepseek-coder-6.7b-instruct. "
                             "Stage 2 default: ./results_deepseek_s1/final_checkpoint")
    ...
    parser.add_argument("--lora_target_layers_start", type=int, default=16,
                        help="Starting layer for LoRA (default 16, matching masking_layer range(16,32))")
    ...
    parser.add_argument("--output_dir", type=str, default=None,
                        help="Default: stage1=./results_deepseek_s1, stage2=./results_deepseek_s2")
    parser.add_argument("--project_name", type=str, default="COREGEN-DeepSeek")
    ...
    if args.model_name is None:
        args.model_name = (os.path.expanduser("~/models/deepseek-coder-6.7b-instruct") if args.stage == 1
                           else "./results_deepseek_s1/final_checkpoint")
    if args.max_train_steps is None:
        args.max_train_steps = 34722 if args.stage == 1 else 3472
    if args.output_dir is None:
        args.output_dir = f"./results_deepseek_s{args.stage}"
...
def main():
    ...
    logger.info(f"=== COREGEN Stage {args.stage} for DeepSeek-Coder-6.7B ===")
    logger.info(f"Accelerator state: {accelerator.state}")

    tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True)

    # Load COREGEN LLaMA model (DeepSeek-Coder-6.7B uses LLaMA architecture)
    base_model = COREGENLlamaForCausalLM.from_pretrained(
