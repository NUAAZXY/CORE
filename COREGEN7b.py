...
@dataclass
class CodeCausalLMOutputWithPast(ModelOutput):
    """
    ...
    sparsity: Optional[Tuple[torch.FloatTensor, ...]] = None
...
class CodeDynamicCache(nn.Module):
    ...
    def find_position_difference(self, n):
        ...
        if n > len(positions):
            ...
            # 或选项2: 抛出更友好的错误
            # raise ValueError(f"Requested position {n} exceeds available positions {len(positions)}")
        else:
    ...
    def update_pos(
        ...
        if len(self.pos_embeddings) <= layer_idx:
            self.pos_embeddings.append(pos_embedding)
        else:
            for idx, pos in enumerate(line_pos):
                if pos[0] == -1:
                    pos_embedding[idx] = self.pos_embeddings[layer_idx][idx, -1]
            self.pos_embeddings[layer_idx] = torch.cat([self.pos_embeddings[layer_idx], pos_embedding], dim=-2)

        return self.pos_embeddings[layer_idx]
    ...
    @classmethod
    def from_legacy_cache(cls, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None) -> "CodeDynamicCache":
    ...
    def __init__(self, act_size, dict_size, block_indices, top_k):
        ...
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.act_size, self.dict_size)
            )
        )
        self.W_dec = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.dict_size, self.act_size)
            )
        )
        ...
        if self.W_enc.size(-1) == 0:
            self.W_dec = nn.Parameter(
                torch.empty(
                    self.dict_size, self.act_size
                )
            )
            self.W_enc = nn.Parameter(
                torch.empty(
                    self.act_size, self.dict_size
                )
            )
...
def dynamic_topk_attention(d_score, training_length=1024, max_k=512, is_prefilling=True, block_size=64):
    ...
    def get_k_for_position(pos):
        ...
        if pos < training_length:
            ...
            # position_ratio = int(pos * (max_k / training_length))
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # self.in_line_layer = [0, 1, 2, 3]
        # self.window_attn_layer = range(4, 16)
        self.masking_layer = range(16, 32)
        ...
        if self.layer_idx == self.masking_layer[0]:
            self.dependency_encoding = DependencyEncoding(self.hidden_size, sum(self.activation_groups),
    ...
    def forward(
        ...
        if self.layer_idx == self.masking_layer[0]:
            if past_key_value is None or (q_len != 1) or \
                ...
                expanded_indices = safe_indices.unsqueeze(-1).expand(-1, -1,
                                                                     hidden_states.size(-1))  # (batch, m, hidden_size)
                gathered = torch.gather(hidden_states, 1, expanded_indices)  # (batch, m, hidden_size)
                line_positions_ = line_positions.clone()
                line_positions_[line_positions_ == q_len] = q_len - 1
                anchor_ids = torch.gather(position_ids[0].repeat(bsz, 1), 1, line_positions_)
                outputs = self.dependency_encoding(gathered, past_key_value)
                encodings = outputs['features']

                repeat_indices = []
                mask = line_positions != -1  # (batch, m)
                for i in range(bsz):
                    valid_indices = line_positions[i][mask[i]]  # 过滤掉padding的索引
                ...
                if self._use_block_mask:
                    ...
                    for dim, (feature_split, block_att_mask) in enumerate(zip(feature_splits, block_att_masks)):
                        ...
                        feature_k = feature_k.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
                        ...
                        causal_mask = torch.triu(torch.ones(q_len, q_len, dtype=torch.bool, device=d_score.device),
                                                 diagonal=1)
                        d_score = d_score + attention_mask[1][: ,: ,:, :q_len]
                        if past_key_value is not None:
                            final_mask = dynamic_topk_attention(d_score, training_length=1024, max_k=900, is_prefilling=q_len!=1)
                        else:
                        ...
                        if past_key_value is not None and q_len == 1 and past_key_value.tokens[0].tolist()[-1] == 13:
                            ...
                            b_mask[:, :, :, -past_key_value.find_position_difference(past_key_value.f[dim].size(-2)):] = True
                        final_mask = torch.logical_and(final_mask, ~b_mask)
                        final_mask = torch.logical_or(final_mask, causal_mask)
                    ...
                    if q_len == 1 and use_cache:
                        ...
                        new_mask[:, :, lbm.size(-1), :lbm.size(-1)] = lbm[:, :,
                                                                      lbm.size(-1) - 1,
                                                                      :lbm.size(-1)]
                        new_mask[:, :, :lbm.size(-1), lbm.size(-1)] = lbm[:, :, :lbm.size(-1),
                                                                      lbm.size(-1) - 1]
        ...
        # cos, sin = self.rotary_emb(value_states, position_ids[0])
        # query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
        if past_key_value is not None and use_cache:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx)
        if q_len == 1:
        # 推理阶段：构建完整的 position_ids
            current_pos = position_ids[0, 0].item()  # 当前 token 的位置
            ...
            # 计算相对位置
            relative_position_ids = (position_ids[:, -1:] - full_position_ids).clip(max=512)

            cos, sin = self.rotary_emb(value_states, relative_position_ids)
            cos = cos.unsqueeze(1)
            sin = sin.unsqueeze(1)
            ...
            # 2. 窗口 RoPE：创建固定窗口位置的 position_ids
            window_position_ids = torch.full_like(position_ids, 512)
            cos_window, sin_window = self.rotary_emb(value_states, window_position_ids)
            query_states2, _ = apply_rotary_pos_emb(query_states, key_states, cos_window, sin_window)
            ...
            rectified_mask = (position_ids[:, -q_len:, None] - position_ids[:, None]).abs() < 512
            attn_weights = torch.where(rectified_mask, attn_weights1, attn_weights2)

        # if self.layer_idx in self.in_line_layer:
        #     attention_mask = attention_mask[0]
        # elif self.layer_idx in self.window_attn_layer:
        #     attention_mask = attention_mask[-1]
        if block_mask is not None:
            attention_mask = attention_mask[1][:,:,:,:q_len] + block_mask
            # attention_mask = attention_mask[1][:, :, :, :q_len]
        else:
            attention_mask = attention_mask[1]
    ...
    #                 d_score = d_score + attention_mask[1][: ,: ,:, :q_len]
    #                 if past_key_value is not None:
    #                     final_mask = dynamic_topk_attention(d_score, training_length=1024, max_k=512, is_prefilling=q_len!=1)
    #                 else:
    #                     final_mask = dynamic_topk_attention(d_score, training_length=1024, max_k=512,
    #                                                         is_prefilling=q_len != 1)
    #                 if past_key_value is not None and q_len == 1 and past_key_value.tokens[0].tolist()[-1] == 13:
    #                     b_mask = torch.zeros(bsz, 1, 1, past_key_value.tokens.size(-1), dtype=bool, device='cuda')
    #                     b_mask[:, :, :, -past_key_value.find_position_difference(past_key_value.f[dim].size(-2)):] = True
    ...
    #     # elif self.layer_idx in self.window_attn_layer:
    #     #     attention_mask = attention_mask[-1]
    #     if self.layer_idx not in self.masking_layer:
    #         attention_mask = attention_mask[1]
    #     else:
    #         if block_mask is not None:
    #             attention_mask = attention_mask[1][:,:,:,:q_len] + block_mask
    ...
    #     value_states = repeat_kv(value_states, self.num_key_value_groups)
    #
    #     # if self.layer_idx in self.window_attn_layer:
    #     #     if past_key_value is None:
    #     #         bias = (self.m * get_relative_positions(q_len)).unsqueeze(0)
    #     #     else:
    #     #         if q_len != 1:
    #     #             bias = self.m * get_relative_positions(past_key_value.tokens.size(-1)).unsqueeze(0)
    #     #         else:
    #     #             bias = self.m * get_relative_positions(past_key_value.tokens.size(-1))
    #     #             bias = bias[:, -1].unsqueeze(0).unsqueeze(-2)
    #     # else:
    #     #     bias = 0.
    #     attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
    #     if attention_mask is not None:  # no matter the length, we just slice it
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if (
                use_cache and not isinstance(past_key_values, CodeDynamicCache) and not self.training
        ...
        deep_mask, anchor_mask = self.construct_mask_matrix(anchor_mask)
        ...
        anchor_mask = (~anchor_mask.bool()).float()
        anchor_mask[anchor_mask.bool()] = torch.finfo(inputs_embeds.dtype).min
        ...
        if not self.training:
            max_indices_per_row = min(int(row_indices.size(0) / input_ids.size(0)), 800)
        ...
        position_ids_ = torch.stack([torch.cat([torch.arange(0, i) for i in ids if i > 0], dim=-1) for ids in indices],
                                    dim=0).clip(0, 127)
        ...
        for decoder_layer in self.layers:
            decoder_layer.self_attn._use_block_mask = self.use_block_mask
    ...
    def window_attention_mask(self, seq_len, window_size):
        # 创建位置索引矩阵
        row_idx = torch.arange(seq_len, device=self.device).unsqueeze(1)  # [seq_len, 1]
        col_idx = torch.arange(seq_len, device=self.device).unsqueeze(0)  # [1, seq_len]

        # Attention sink: 前4个token对所有位置都可见
        sink_mask = col_idx < 1

        # 滑动窗口mask
        # 每个位置i可以attend到最近window_size个token（从位置4之后开始应用窗口限制）
        # 窗口范围: [max(4, i - window_size + 1), i]
        window_start = torch.clamp(row_idx - window_size + 1, min=4)
        window_mask = (col_idx >= window_start) & (col_idx <= row_idx)
        # 合并attention sink和滑动窗口mask
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            ...
            return (loss_final,) + output if loss_final is not None else output

        return CodeCausalLMOutputWithPast(
            loss=loss_final,
            logits=logits,
