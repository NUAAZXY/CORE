...
from COREGEN7b import LlamaForCausalLM
...
class LoRALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Ensure LoRA parameters match input dtype
...
def inject_lora_to_model(model, target_layers_start=16, rank=32, target_modules=['o_proj', 'v_proj']):
    """
    Inject LoRA layers into the model for specified modules in layers >= target_layers_start

    Args:
        model: The LLaMA model
        target_layers_start: Starting layer index for LoRA (default: 16)
        rank: LoRA rank (default: 32)
        target_modules: List of module names to apply LoRA (default: ['o_proj', 'v_proj'])
    """
    ...
    # Unfreeze DependencyEncoding parameters if they exist
    logging.info("Unfreezing DependencyEncoding parameters...")
    dependency_param_count = 0
    for name, module in model.named_modules():
        if 'DependencyEncoding' in str(type(module)):
            for param_name, param in module.named_parameters():
                param.requires_grad = True
                dependency_param_count += param.numel()
                logging.info(f"Unfroze DependencyEncoding parameter: {name}.{param_name}, shape: {param.shape}")

    if dependency_param_count > 0:
        logging.info(f"Total DependencyEncoding parameters unfrozen: {dependency_param_count:,}")
    else:
        logging.info("No DependencyEncoding parameters found")

    lora_layers = {}

    # Iterate through model layers
    for name, module in model.named_modules():
        # Check if this is a target layer (>= 16) and target module (o_proj or v_proj)
        if 'layers' in name and isinstance(module, nn.Linear):
            # Extract layer index - handle different naming patterns
            parts = name.split('.')

            try:
                # Find 'layers' in the parts and get the next element as layer index
            ...
            # Only apply LoRA to layers >= target_layers_start
            if layer_idx >= target_layers_start:
                # Check if this is o_proj or v_proj
                ...
                if module_name in target_modules:
                    ...
                    logging.info(f"Injected LoRA into {name} (layer {layer_idx}, in_features={module.in_features}, out_features={module.out_features})")
    ...
    # Register LoRA layers as model parameters (these will be trainable)
    for name, lora_layer in lora_layers.items():
        # Convert name to valid parameter name
...
class LoRAModel(nn.Module):
    """Wrapper to apply LoRA during forward pass"""
    ...
    def merge_lora_weights(self):
        ...
        for name, lora_layer in self.lora_layers.items():
            ...
            if isinstance(module, nn.Linear):
                # Calculate LoRA update: delta_W = B @ A * scaling
                # Ensure dtype matches the original weight
    ...
    def save_pretrained(self, save_path, merge_lora=True, **kwargs):
        ...
        Args:
            ...
            merge_lora: If True, merge LoRA weights into base model and save complete model.
                       If False, only save LoRA weights separately.
        """
        ...
        if merge_lora:
            ...
            # Save the merged model using HuggingFace's save_pretrained
            ...
            # Save only LoRA weights (original behavior)
            lora_state_dict = {}
            for name, lora_layer in self.lora_layers.items():
...
def get_lora_params(model, args):
    ...
    for name, param in model.named_parameters():
        if param.requires_grad and ('lora_A' in name or 'lora_B' in name):
            ...
            logging.info(f"Training LoRA parameter: {name}, shape: {param.shape}")

    # No weight decay for LoRA parameters (common practice)
    return [{'params': lora_params, 'weight_decay': 0.0}]
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train language model with COPE and LoRA")
    ...
    parser.add_argument("--merge_lora_on_save", action="store_true", help="Merge LoRA weights into model when saving (default: True)", default=True)
    ...
    parser.add_argument("--max_train_steps", type=int, default=163000, help="Maximum training steps")
    parser.add_argument("--max_eval_steps", type=int, default=1, help="Maximum evaluation steps")

    # Data parameters
    parser.add_argument("--train_dataset", type=str, default="/home/user/PycharmProjects/CoPE/starcoder_20Btokens", help="Training dataset path")
    ...
    parser.add_argument("--save_checkpoint_steps", type=int, default=10000, help="Save checkpoint steps")
...
def main():
    ...
    # 显式重新初始化 DependencyEncoding 参数f
    for name, module in base_model.named_modules():
        if 'dependency_encoding' in name.lower():
            if hasattr(module, 'W_enc'):
                nn.init.kaiming_uniform_(module.W_enc)
                logging.info(f"Re-initialized {name}.W_enc with shape {module.W_enc.shape}")
            if hasattr(module, 'W_dec'):
                nn.init.kaiming_uniform_(module.W_dec)
                logging.info(f"Re-initialized {name}.W_dec")
    ...
    # Setup optimizer and scheduler - only optimize LoRA parameters
    optimizer = AdamW(get_lora_params(model, args), lr=args.learning_rate)
    ...
    for step, batch in enumerate(tqdm(train_dataloader,
        ...
        if step % args.save_checkpoint_steps == 0 and step > 0:
            ...
            # Decide whether to merge LoRA or save separately
